...
contract GatherSYL is Ownable {
    ...
    function staking(address _nfts, uint256 _sylId) external {
        ...
        require(nfts.ownerOf(_sylId) == msg.sender, "not owner");
        console.log(nfts.ownerOf(_sylId));
        nfts.transferFrom(msg.sender, address(this), _sylId);
        nftStaked[_nfts][msg.sender].push(_sylId);
    }
    ...
    function withdraw(address _nfts, uint256 _sylId) external {
        ...
        require(stakeAllowed[msg.sender]);
        IKIP17Enumerable nfts = IKIP17Enumerable(_nfts);
        require(nfts.ownerOf(_sylId) == address(this), "not owner");
        for (
            uint256 k = 0;
            k < nftStaked[_nfts][msg.sender].length;
            k = k.add(1)
        ) {
            if (nftStaked[_nfts][msg.sender][k] == _sylId) {
                delete nftStaked[_nfts][msg.sender][k];
                nfts.transferFrom(address(this), msg.sender, _sylId);
            }
        }
    }
